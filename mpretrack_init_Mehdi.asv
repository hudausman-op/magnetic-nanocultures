function [M2,MT] = mpretrack_init_Mehdi(basepath,inv, featsize, barint, barrg, barcc, IdivRg, fovn, frame, masscut, Imin, field)
...
% img=imread([pathin 'fov' num2str(fovn) '\fov' num2str(fovn) '' num2str(frame,'%i') '.tif']);
% img=imread([pathin 'fov' num2str(fovn) '\fov1_' num2str(frame,'%04d') '.tif']);
% img=imread([pathin 'Recorder-Stream-',num2str(fovn),'\Recorder-Stream-' num2str(fovn) '_t' num2str(frame,'%04d') '.tif']);
% img=imread([pathin '/Img_2018-08-15-122228-' num2str(frame,'%04i') '.tif']);
% img=imread([pathin 'Img' num2str(frame,'%04i') '.tif']);
img=imread(sprintf(pathin,frame));

% img=imread([pathin 'fov' num2str(fovn) '\Img_T_' num2str(frame,'%04i') '.tif']);
...
img2=img;
    ...
    %This is to mask a circle in center
%     sz=size(img);
